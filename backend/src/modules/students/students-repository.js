...
const addOrUpdateStudent = async (payload) => {
    ...
    // Execute the database function and return the result
    const { rows } = await processDBRequest({ query, queryParams });
    return rows[0];
}
...
// This will delete both the user record and associated user_profile record
// Uses CASCADE delete if foreign key constraints are set up, otherwise deletes manually
const deleteStudent = async (id) => {
    // First delete the user_profile (child record)
    ...
    // Then delete the user record (parent record)
